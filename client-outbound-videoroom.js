...
const {
  JoinRoom, makeOutboundCall, hangupCall,
} = require('./voiceapi');
...
function voiceEventHandler(voiceEvent) {
  if (voiceEvent.state) {
    if (voiceEvent.state === 'connected') {
      ...
      const eventMsg = 'Outbound Call Joined Video Room';
      logger.info(`[${callVoiceId}] ${eventMsg}`);
      sseMsg.push(eventMsg);
  ...
  if (voiceEvent.playstate !== undefined) {
    if (voiceEvent.playstate === 'playfinished') {
      if (voiceEvent.prompt_ref === '1') {
        const eventMsg = 'Greeting is completed, redirecting the call to Video Room';
        logger.info(`[${callVoiceId}] ${eventMsg}`);
        sseMsg.push(eventMsg);
        /* Join Video Room*/
        JoinRoom(callVoiceId , room_id, () => {});
